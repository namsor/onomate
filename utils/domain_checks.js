...
class DomainChecker {
     ...
     */
    async checkTLDVariations(baseName, tlds = ['.com', '.io', '.co', '.net', '.org']) {
     ...
     */
    async findAlternatives(baseName, maxAlternatives = 10) {
    ...
    async performDomainCheck(domain) {
    ...
    generateAlternatives(baseName, count) {
    ...
    containsValuableKeywords(domain) {
    ...
    createErrorResult(domain, error) {
        return {
            domain,
            available: false,
            lastChecked: new Date().toISOString(),
     ...
     */
    async checkDomainsForNames(names) {
        const results = {};
     ...
     */
    async findBestDomain(name, preferences) {
...
//# sourceMappingURL=domain_checks.js.map