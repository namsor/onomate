...
class NameFilters {
     ...
     */
    static filterByConstraints(names, criteria) {
        return names.filter(name => {
            // Length filters
            if (criteria.minLength && name.length < criteria.minLength)
                return false;
            if (criteria.maxLength && name.length > criteria.maxLength)
                return false;
     ...
     */
    static rankNames(names, criteria) {
        return names
            .map(name => ({
            name,
     ...
     */
    static assessTrademarkRisk(name, industry) {
     ...
     */
    static suggestDomainVariations(name) {
        const variations = [];
    ...
    static calculateMemoryScore(name) {
        ...
        if (rhythm)
            score += 0.2;
        // Familiar patterns
        if (this.hasFamiliarPatterns(name))
            score += 0.1;
    ...
    static calculateBrandabilityScore(name) {
        let score = 0.5;
        // Uniqueness (not a dictionary word)
        if (!this.isDictionaryWord(name))
            score += 0.2;
        // Visual appeal
        if (this.hasVisualAppeal(name))
            score += 0.1;
        // Evokes emotion or meaning
        if (this.evokesPositiveAssociation(name))
            score += 0.2;
        // Scalability (not too specific)
        if (!this.isTooSpecific(name))
            score += 0.1;
        return Math.max(0, Math.min(1, score));
    }
    static calculateInternationalViability(name) {
        let score = 0.8; // Start optimistic
        // Check for letters that don't exist in many languages
        if (/[jqvwx]/i.test(name))
            score -= 0.1;
        // Check for difficult combinations for non-English speakers
        if (/th|ch|sh|gh/i.test(name))
            score -= 0.05;
    ...
    static identifyLinguisticRisks(name) {
        const risks = [];
    ...
    static isPhoneticallyPleasant(name) {
    ...
    static analyzeCulturalRisks(name) {
        const risks = [];
        const lowerName = name.toLowerCase();
    ...
    static calculateOverallScore(name, criteria) {
    ...
    static evokesPositiveAssociation(name) {
...
exports.default = NameFilters;
//# sourceMappingURL=name_filters.js.map